...
serve(async (req) => {
  ...
  try {
    ...
    const adults = params.guests.filter((g) => g.type === "adult").length || 1;
    const nights = countNights(params.check_in_date, params.check_out_date);

    console.log(
      `Hotel search: ${params.destination} | ${params.check_in_date} → ${params.check_out_date} | ${params.rooms} room(s) | ${adults} adult(s) | ${nights} night(s)`
    );

    const token = await getAmadeusToken(clientId, clientSecret);

    // Step 1: Resolve IATA code → city-centre coordinates
    let coords: { latitude: number; longitude: number };
    try {
      coords = await resolveCoordinates(params.destination, token);
      console.log(`Resolved ${params.destination} → lat:${coords.latitude} lon:${coords.longitude}`);
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Unknown error";
      console.error("Coordinate resolution failed:", msg);
      return new Response(
        JSON.stringify({ error: `Could not resolve destination: ${msg}`, hotels: [] }),
        { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    // Step 2: Get hotel IDs near coordinates
    const geocodeUrl = new URL(
      `${AMADEUS_BASE}/v1/reference-data/locations/hotels/by-geocode`
    );
    geocodeUrl.searchParams.set("latitude", String(coords.latitude));
    geocodeUrl.searchParams.set("longitude", String(coords.longitude));
    geocodeUrl.searchParams.set("radius", "20");
    geocodeUrl.searchParams.set("radiusUnit", "KM");
    ...
    if (!geocodeResp.ok) {
      const errText = await geocodeResp.text();
      console.error(`Hotel geocode search error [${geocodeResp.status}]: ${errText}`);
      return new Response(
        JSON.stringify({ error: `Hotel search failed (${geocodeResp.status})`, hotels: [] }),
        { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    const geocodeData = await geocodeResp.json();
    const hotelIds: string[] = (geocodeData.data || [])
      .slice(0, 25)
      .map((h: any) => h.hotelId);

    console.log(`Found ${hotelIds.length} hotels near ${params.destination}`);

    if (hotelIds.length === 0) {
      return new Response(
        JSON.stringify({
          hotels: [],
          message: `No hotels found near ${params.destination}`,
        }),
        { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    // Step 3: Get hotel offers for those IDs
    const offersUrl = new URL(`${AMADEUS_BASE}/v3/shopping/hotel-offers`);
    offersUrl.searchParams.set("hotelIds", hotelIds.join(","));
    offersUrl.searchParams.set("checkInDate", params.check_in_date);
    offersUrl.searchParams.set("checkOutDate", params.check_out_date);
    offersUrl.searchParams.set("adults", String(adults));
    offersUrl.searchParams.set("roomQuantity", String(params.rooms));
    ...
    if (!offersResp.ok) {
      ...
      console.error(`Hotel offers error [${offersResp.status}]: ${errText}`);
      return new Response(
        JSON.stringify({ error: `Hotel offers failed (${offersResp.status})`, hotels: [] }),
        { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }

    const offersData = await offersResp.json();
    console.log(`Received ${offersData.data?.length ?? 0} hotel offers from Amadeus`);

    const hotels = (offersData.data || [])
      .filter((h: any) => h.offers?.length > 0)
      .map((h: any) => {
        const hotel = h.hotel;
        const offer = h.offers[0];
        const total = parseFloat(offer.price.total);
        const nightly = (total / nights).toFixed(2);

        return {
          property_id:      hotel.hotelId,
          search_result_id: offer.id,
          name:             hotel.name,
          ...
          photo_url:        hotel.media?.[0]?.uri || null,
    ...
    // Sort by total price, return top 1
    hotels.sort(
      (a: any, b: any) => parseFloat(a.total_amount) - parseFloat(b.total_amount)
    );
    const topHotels = hotels.slice(0, 1);

    console.log(`Returning top ${topHotels.length} hotel offers`);

    return new Response(
      JSON.stringify({ hotels: topHotels }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error: unknown) {
    console.error("Error in search_hotels_amadeus:", error);
    const msg = error instanceof Error ? error.message : "Unknown error";
    return new Response(
      JSON.stringify({ error: msg, hotels: [] }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  }
});
